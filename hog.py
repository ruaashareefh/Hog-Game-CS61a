...
def make_averaged(fn, num_samples=1000):
    ...
    """
    def average_fn_value(*args): 
        sum = 0
        for i in range(0, num_samples):
            sum += fn(*args) 
        return sum/num_samples

    return average_fn_value
...
def max_scoring_num_rolls(dice=six_sided):
    ...
    for rolls in range(1, 11):
        curr_mean = make_averaged(roll_dice, num_samples=10000)(rolls, dice)
        if curr_mean > max_mean:
            max_mean = curr_mean
            max_rolls = rolls
        print(rolls, 'dice scores', curr_mean, 'on average')
    return max_rolls
